...
local DEFAULT_L3_COUNT = 3
local DEFAULT_L3_LENGTH = 72
...
local function choose_l3_mode(p, pocket_id)
  local jitter = ((pocket_id * 17 + p.id) % 100) / 100.0
  if p.calm_coherence + jitter * 0.04 > 0.90 and p.calm_mass > 3.2 then
    return "MANIFEST"
  elseif p.raw_noise - jitter * 0.03 > 0.58 then
    return "LOGIC"
  elseif p.chaos_flux + jitter * 0.06 > 0.70 then
    return "CYCLE"
  end
  return "RUNTIME"
end
...
local function make_l3(length, seed, pocket_id, l3_count)
  local rng = make_rng(seed * 17 + pocket_id * 97 + 9)
  local cells = {}
  for i = 1, length do
    cells[i] = {
      activation = 0.0,
  ...
  return {
    ...
    manifest = 0.0,
    pu_burned = 0.0,
    phase = (pocket_id - 1) * 0.23,
    cycle_speed = 0.12 + (pocket_id - 1) * 0.01,
    modes = { RUNTIME = 0, CYCLE = 0, LOGIC = 0, MANIFEST = 0 },
    cost_scale = 1.0 / l3_count,
  }
end
...
local function apply_crystal(state, crystal)
  ...
  for _, idx in ipairs(l3_indices(state, crystal)) do
    ...
    elseif crystal.mode == "CYCLE" then
      cell.charge = cell.charge + math.sin(state.phase + idx * 0.09 + state.id * 0.2) * e * 0.45
...
local function crystal_burn(state, crystal)
  local base = 0.30
  local mode = ({
    RUNTIME = 0.14,
    CYCLE = 0.16,
  ...
  return (base + mode + crystal.energy * 0.20) * state.cost_scale
end
...
local function l3_step(state)
  ...
  for _, crystal in ipairs(state.active) do
    ...
    local burn = crystal_burn(state, crystal)
  ...
  for i = 1, state.length do
    ...
    elseif c.mode == "CYCLE" then
      raw = raw + math.sin(state.phase + i * 0.10 + state.id * 0.15) * 0.06
    end
...
local function route_to_l3(p, tick, l3_count)
  return 1 + ((p.id * 11 + tick * 7 + p.x * 3 + p.y) % l3_count)
end

local function make_crystal(p, tick, l3)
  local mode = choose_l3_mode(p, l3.id)
  l3.modes[mode] = l3.modes[mode] + 1
  return {
    mode = mode,
    target = 1 + ((p.id * 7 + tick * 3 + l3.id * 13) % l3.length),
...
local function main()
  ...
  local l3_count = tonumber(arg[4]) or DEFAULT_L3_COUNT
  local l3_length = tonumber(arg[5]) or DEFAULT_L3_LENGTH
  ...
  local l3_list = {}
  for i = 1, l3_count do
    l3_list[i] = make_l3(l3_length, seed, i, l3_count)
  end
  ...
  for tick = 1, ticks do
    for i = 1, process_count do
      ...
      if emitted and not p.emitted then
        p.emitted = true
        local pocket = route_to_l3(p, tick, l3_count)
        local l3 = l3_list[pocket]
        l3.active[#l3.active + 1] = make_crystal(p, tick, l3)
      end
    end
    for i = 1, l3_count do
      l3_step(l3_list[i])
  ...
  print(string.format(
    "Eva.00 multi-L3 cost stand :: ticks=%d processes=%d seed=%d l3_count=%d l3_length=%d",
    ticks, process_count, seed, l3_count, l3_length
  ))
  print(string.format(
    "processes :: alive=%d success=%d failed=%d runtime=%d",
    alive, success, failed, runtime_calls
  ))
  print(string.format(
    "process_avg :: age=%.2f burn=%.2f coherence=%.4f",
  ...
  local total_exhausted = 0
  local total_manifest = 0.0
  local total_l3_burned = 0.0
  local total_readout = 0.0
  local total_modes = { RUNTIME = 0, CYCLE = 0, LOGIC = 0, MANIFEST = 0 }

  for i = 1, l3_count do
    local l3 = l3_list[i]
    local active_modes = active_count_by_mode(l3.active)
    local readout = l3_readout(l3)
    total_active = total_active + #l3.active
    total_exhausted = total_exhausted + #l3.exhausted
    total_manifest = total_manifest + l3.manifest
    total_l3_burned = total_l3_burned + l3.pu_burned
    total_readout = total_readout + readout
    total_modes.RUNTIME = total_modes.RUNTIME + l3.modes.RUNTIME
    total_modes.CYCLE = total_modes.CYCLE + l3.modes.CYCLE
    total_modes.LOGIC = total_modes.LOGIC + l3.modes.LOGIC
    total_modes.MANIFEST = total_modes.MANIFEST + l3.modes.MANIFEST
    ...
    print(string.format(
      ...
      l3.modes.LOGIC,
      l3.modes.MANIFEST,
      active_modes.RUNTIME,
      active_modes.CYCLE,
      active_modes.LOGIC,
      active_modes.MANIFEST
    ))
  end
