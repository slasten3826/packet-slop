...
__device__ static int choose_l3_mode_dev(const EncodeProcess& p) {
    if (p.calm_coherence > 0.88f && p.calm_mass > 3.2f) return M_MANIFEST;
    if (p.raw_noise > 0.56f) return M_LOGIC;
    if (p.chaos_flux > 0.70f) return M_CYCLE;
    return M_RUNTIME;
}
...
__global__ void tick_encode_kernel(
    ...
    if (p.calm_mass > 2.2f && p.calm_coherence > 0.68f && p.connect_strength > 0.25f) {
        ...
        Crystal c;
        c.mode = choose_l3_mode_dev(p);
        c.target = (p.id * 7 + tick * 3) % l3_length;
...
__global__ void update_l3_kernel(L3Cell* cells, int length, float phase) {
    ...
    else if (c.mode == M_CYCLE) raw = raw + sinf(phase + idx * 0.10f) * 0.06f;
...
static void init_l3(std::vector<L3Cell>& out, int length, int seed) {
    out.resize(length);
    unsigned int rng = (unsigned int)(seed * 17 + 9);
...
int main(int argc, char** argv) {
    ...
    int l3_length = argc > 4 ? std::atoi(argv[4]) : 72;

    std::vector<EncodeProcess> h_processes;
    std::vector<L3Cell> h_l3;
    init_processes(h_processes, process_count, seed);
    init_l3(h_l3, l3_length, seed);
    ...
    std::printf("cuda eva00 encode-core + l3 stand\n");
    std::printf("ticks=%d processes=%d l3=%d seed=%d\n", ticks, process_count, l3_length, seed);

    for (int tick = 1; tick <= ticks; ++tick) {
        check_cuda(cudaMemset(d_emitted_count, 0, sizeof(int)), "memset emitted_count");
        tick_encode_kernel<<<process_blocks, threads>>>(d_processes, process_count, tick, seed, d_emitted, d_emitted_count, l3_length);
        ...
        if (emitted_count > 0) {
            ...
            for (int i = 0; i < emitted_count; ++i) {
                active.push_back(emitted[i]);
                incoming_modes[emitted[i].mode] += 1;
            }
        }
        ...
        if (!active.empty()) {
            ...
            check_cuda(cudaMemcpy(d_active, active.data(), sizeof(Crystal) * active.size(), cudaMemcpyHostToDevice), "copy active crystals");
            ...
            int crystal_blocks = ((int)active.size() + threads - 1) / threads;
            apply_l3_crystals_kernel<<<crystal_blocks, threads>>>(d_l3, l3_length, d_active, (int)active.size(), phase, d_manifest_delta, d_burned);
            check_cuda(cudaGetLastError(), "apply_l3_crystals");
            check_cuda(cudaMemcpy(active.data(), d_active, sizeof(Crystal) * active.size(), cudaMemcpyDeviceToHost), "copy active crystals back");
        ...
        phase += 0.12f;
        update_l3_kernel<<<l3_blocks, threads>>>(d_l3, l3_length, phase);
        check_cuda(cudaGetLastError(), "update_l3_kernel");
        commit_l3_kernel<<<l3_blocks, threads>>>(d_l3, l3_length);
        check_cuda(cudaGetLastError(), "commit_l3_kernel");
        check_cuda(cudaDeviceSynchronize(), "sync l3");
        ...
        for (const Crystal& c : active) {
            ...
            else exhausted_total += 1;
        }
        active.swap(survivors);

        if (tick == 1 || tick % std::max(1, ticks / 8) == 0 || tick == ticks) {
            int active_modes[4] = {0, 0, 0, 0};
            float active_pu = 0.0f;
            for (const Crystal& c : active) {
                active_modes[c.mode] += 1;
                active_pu += fmaxf(0.0f, c.pu_stock);
            }
            std::printf(
                "[tick=%d] emitted=%d active=%zu exhausted=%d in=(R:%d C:%d L:%d M:%d) act=(R:%d C:%d L:%d M:%d) readout=%.4f manifest=%.3f active_pu=%.1f burned=%.1f\n",
                tick,
                emitted_count,
                active.size(),
                exhausted_total,
                incoming_modes[M_RUNTIME], incoming_modes[M_CYCLE], incoming_modes[M_LOGIC], incoming_modes[M_MANIFEST],
                active_modes[M_RUNTIME], active_modes[M_CYCLE], active_modes[M_LOGIC], active_modes[M_MANIFEST],
                readout_host(h_l3),
                manifest,
                active_pu,
                burned_total
    ...
    int active_modes[4] = {0, 0, 0, 0};
    for (const Crystal& c : active) active_modes[c.mode] += 1;
    ...
    std::printf("l3 :: active=%zu exhausted=%d readout=%.4f manifest=%.3f burned=%.1f\n",
        active.size(), exhausted_total, readout_host(h_l3), manifest, burned_total
    );
    std::printf("l3_incoming_modes :: RUNTIME=%d CYCLE=%d LOGIC=%d MANIFEST=%d\n",
        incoming_modes[M_RUNTIME], incoming_modes[M_CYCLE], incoming_modes[M_LOGIC], incoming_modes[M_MANIFEST]
    );
    std::printf("l3_active_modes :: RUNTIME=%d CYCLE=%d LOGIC=%d MANIFEST=%d\n",
        active_modes[M_RUNTIME], active_modes[M_CYCLE], active_modes[M_LOGIC], active_modes[M_MANIFEST]
    );
