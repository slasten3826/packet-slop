...
local function spawn_encoded_seed(state, tick, x, y, raw, coherence)
    ...
    for x = 1, state.l2_width do
      ...
      elseif cell.kind == "ENCODE" then
        ...
        if cell.stability >= 3 and surplus > 0.05 then
          ...
          spawn_encoded_seed(state, tick, x, y, raw, coherence)
          cell.stability = 1
...
local function l3_seed_burn(seed)
  return 0.16 + seed.energy * 0.26
end

local function apply_l3_seed(state, seed)
  local cell = state.l3[seed.target]
  local strength = clamp(seed.pu_stock / math.max(1.0, seed.pu_initial), 0.10, 1.0)
  local e = seed.energy * strength
  cell.charge = cell.charge + e * 0.80
  cell.coherence = clamp(cell.coherence + seed.coherence * 0.10, 0.0, 1.0)
  cell.load = cell.load + e * 0.18
  cell.gate = clamp(cell.gate + e * 0.04, 0.0, 1.0)
end

local function dominant_kind(cell)
  if cell.runtime_field >= cell.logic_field and cell.runtime_field >= cell.cycle_field then
    return "RUNTIME"
  elseif cell.logic_field >= cell.cycle_field then
    return "LOGIC"
  end
  return "CYCLE"
end
...
local function step_l3(state)
  ...
  local survivors = {}
  for _, seed in ipairs(state.l3_active) do
    apply_l3_seed(state, seed)
    local burn = l3_seed_burn(seed)
    seed.pu_stock = seed.pu_stock - burn
    seed.age = seed.age + 1
    state.metrics.burned = state.metrics.burned + burn
    if seed.pu_stock > 0 then
      survivors[#survivors + 1] = seed
    else
      state.l3_exhausted[#state.l3_exhausted + 1] = seed
    end
  end
  ...
  state.metrics.runtime_cells = 0
  state.metrics.logic_cells = 0
  state.metrics.cycle_cells = 0
  ...
  for i = 1, state.l3_length do
    ...
    local oscillation = math.abs(cell.activation - cell.prev_activation)
    local conflict = math.abs(left.activation - right.activation) + math.abs(raw - cell.activation)
    local persistence = math.max(0.0, cell.activation - oscillation) * (0.6 + cell.coherence * 0.4)

    cell.cycle_field = clamp(cell.cycle_field * 0.90 + oscillation * 0.80 + math.abs(cell.charge) * 0.06, 0.0, 10.0)
    cell.logic_field = clamp(cell.logic_field * 0.90 + conflict * 0.65 + cell.gate * 0.08, 0.0, 10.0)
    cell.runtime_field = clamp(cell.runtime_field * 0.92 + persistence * 0.55 + cell.coherence * 0.05 - conflict * 0.08, 0.0, 10.0)

    local kind = dominant_kind(cell)
    if kind == "RUNTIME" then
      state.metrics.runtime_cells = state.metrics.runtime_cells + 1
      local maintenance = (0.46 + cell.load * 0.020 + cell.runtime_field * 0.030)
      cell.load = math.max(0.0, cell.load - maintenance)
      state.metrics.runtime_burned = state.metrics.runtime_burned + maintenance
      state.manifest = state.manifest + math.min(0.03, cell.runtime_field * 0.002)
    elseif kind == "LOGIC" then
      state.metrics.logic_cells = state.metrics.logic_cells + 1
      raw = raw * (0.78 + math.min(0.10, cell.logic_field * 0.01))
      cell.gate = clamp(cell.gate + 0.02, 0.0, 1.0)
    else
      state.metrics.cycle_cells = state.metrics.cycle_cells + 1
      raw = raw + math.sin(state.l3_phase + i * 0.11) * 0.08
    end

    cell.prev_activation = cell.activation
    cell.next_activation = clamp(raw * decay, 0.0, 1.0)
    cell.charge = cell.charge * 0.80
    cell.gate = cell.gate * 0.90
    cell.coherence = clamp(cell.coherence * 0.992 + persistence * 0.01, 0.0, 1.0)
  end
...
local function l3_energy(state)
  ...
  for i = 1, state.l3_length do
    local cell = state.l3[i]
    sum = sum + math.abs(cell.activation) + math.abs(cell.charge) + math.abs(cell.load)
  end
  return sum
end
...
print("l1_l2_l3_emergent_field stand")
...
for tick = 1, l23_ticks do
  ...
  if tick == 1 or tick == l23_ticks or tick % 16 == 0 then
    print(string.format(
      "tick=%d encoded=%d active=%d exhausted=%d cycle_cells=%d logic_cells=%d runtime_cells=%d readout=%.4f energy=%.2f manifest=%.3f",
      tick,
      state.metrics.encoded,
      #state.l3_active,
      #state.l3_exhausted,
      state.metrics.cycle_cells,
      state.metrics.logic_cells,
...
print("")
print(string.format(
  "Totals :: collapse=%d encoded=%d spawned=%d exhausted=%d burned=%.1f runtime_burned=%.1f readout=%.4f manifest=%.3f",
